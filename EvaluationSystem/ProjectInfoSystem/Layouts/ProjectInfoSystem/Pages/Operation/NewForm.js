...
app.controller('opCtrl', function ($scope, service, $q) {
    ...
    $scope.showOperations = function () {
        ...
        $scope.loading = true;
            ...
            service.isExitSameWeeklyReport($scope.contract.Id, $scope.period.Id).then(function (result) {

                $scope.isExitSameReport = result.data.d;

                if ($scope.isExitSameReport) {
                    alert('برای این پیمان قبلا در این دوره گزارش هفتگی ثبت شده است.');
    ...
    $scope.save = function (isTemp) {
        ...
        service.save(weeklyItem,isTemp).then(function (d) {
            $scope.loading = false;
            if (d.data.d == "") {
